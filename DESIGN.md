...
- **Forecast discovery cards (Featured + Browse):** Shared **shell** (`rounded-xl`, `bg-surface-elevated`, `p-5`, unified shadow). **Header:** source avatar + name link (left) with **header end** slot — Browse: **outcome badge as filter action** (per-outcome hover tint; **no** per-card pressed ring — filter state lives in **Showing: {status}** + **Clear status filter** with `role="status"`; **44px** min target; keyboard focus ring only on `:focus-visible`); Featured: compact **{accuracy}% accurate** badge (source accuracy + trend color — **not** clickable, **not** market odds). **Title** links to prediction detail — **not** a whole-card link. **Browse only:** muted **Resolved …** / **Closed …** / **Target …** / **Added …** line sits **below the title, above the footer** (not in the footer), in priority order: **Resolved** for scored terminal rows (correct/incorrect) with `resolved_at`; **Closed** for other terminal rows (unresolved/invalid) with `resolved_at`; **Target** when the forecast names a date and is still open (or terminal without `resolved_at`); **Added** for pending rows without target. **Footer meta row:** muted text-only **topic** link(s) to `/[slug]` from `topicIds` (primary + optional second link / +N). Active outcome filter shows **Showing: {status}** with **Clear status filter** under the Browse heading.
- **Featured forecasts (home hero):** Responsive **single-row** card grid inside the elevated hero shell. **Trending topics** strip sits in the hero header: each item is a **curated topic** (ranked by linked prediction activity), linking to `/[slug]`. **Slot count tracks viewport:** 1 card (&lt; `lg`), 3 (`lg`–`1279px`), 4 (`xl`+); never wrap a second row. Section title: **Featured forecasts** (distinct from the main list **Browse forecasts** below), with one-line **source accuracy / not market prices** disclosure when cards are shown. Pick logic favors **newest per bucket topic** then backfill; copy must not imply popularity or live prices.
- **Topic feed pages:** `/[slug]` (bucket or curated) uses an **8+4 grid** at `lg`: main column is a **vertical** `PredictionFeedList` plus **Browse forecast** card; sidebar (`FeedSidebar`) holds **bucket pills**, **trending topics**, **recent resolutions**, and **platform stats**. Curated pages add a **page header** (name + parent bucket chips). Filters and lists must respect `prefers-reduced-motion` on any entrance animation.
