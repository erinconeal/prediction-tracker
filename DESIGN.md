...
- **Source detail pages:** `/source/[slug]` uses the same **8+4 grid** at `lg` (timeline main, stats sidebar). **Header:** breadcrumb (`Home / {name}` with `aria-current="page"` on the visible current item), **SourceAvatar** + serif **`h1`** + compact **SourceAccuracyBadge** beside the name (track record at first scan; not market odds). **Sidebar** (`SourceStatsSidebar`, sticky at `lg`): accuracy card with mono percent, **LeaderboardAccuracyBar**, constitution §7.3 context line (includes scored count), **total**, **Still open**, and **No longer open** counts (optional **InfoPopover** on still open / no longer open), **How we score** → `/about#lifecycle-language`. **Main column:** **Prediction feed** (`h2`) with **All** / **Still open** status filter pills, optional **Showing: {status}** + **Clear status filter** when filtered; vertical list of **Browse forecast** cards with **`hideSourceHeader`** + **`readOnlyOutcome`**, optional muted **Submitted** / **Finished** date line under the title (still open → `created_at`; terminal → `finished_at` when present), **Load more** pagination (20-row pages via `usePredictionFeed`). Sidebar stats use a separate unfiltered snapshot (`usePredictions`, limit 100); when the snapshot is full, **total** reads **100+** with muted disclosure that counts use the first 100 predictions. The feed list respects the active status filter and paginates independently. Empty slug-backed names use **humanized** slug fallback. Outcome badges on feed cards are read-only; recording outcomes is reserved for a future **admin** role (not inline on this page).
- **Prediction detail pages:** `/predictions/[id]` uses the same **8+4 grid** at `lg` as source and topic feeds. **Breadcrumb:** `Home / {truncated title}` with `aria-current="page"`. **Header card** (`rounded-xl`, elevated): **OutcomeBadge**, primary **topic** pill link, serif **`h1`**, metrics row (**Submitted**, **Target** when set, **Finished** when terminal) in mono tabular dates. **Main column:** lifecycle **Timeline** — **still-open:** Added / Outcome / Target (Outcome before future Target; omit Target when unset); **terminal:** Added / Target / Finished / Outcome (omit Target or Finished when unset). Outcome step shows **OutcomeBadge** only; constitution detail lives in **How we score** → `/about#lifecycle-language`. **No outcome-record controls** on this page until an authenticated **admin** role exists (Mark correct / incorrect / unresolved / invalid are deferred — not community voting). **Sidebar** (sticky at `lg`): source profile card (**SourceAvatar**, name link, **SourceAccuracyBadge**, track-record disclaimer, **Full source profile** CTA) plus **SourceStatsSidebar** snapshot stats (includes **How we score**). No consensus bars or market-style Yes/No controls.
...
| 2026-06-03 | Source detail feed pagination | Prediction feed uses **`usePredictionFeed`** (20-row pages, **Load more**); sidebar stats remain on a separate unfiltered snapshot fetch. |
| 2026-06-04 | Source feed card dates | Source prediction feed cards show one muted line: **Submitted · {date}** while still open, **Finished · {date}** after a final outcome (absolute dates; omitted when dates are missing or invalid). Home/topic browse cards stay date-free. |
| 2026-06-04 | Source stats snapshot cap copy | Full snapshot (100 rows) shows **100+** on total plus disclosure that sidebar counts use the first 100 predictions. |
| 2026-06-04 | Prediction detail page layout | `/predictions/[id]` follows **8+4** grid: elevated header card with lifecycle metrics, main-column timeline, sticky source profile sidebar with **SourceStatsSidebar**; outcome recording deferred until admin auth; no community voting or market-style controls. |
| 2026-06-05 | Prediction detail timeline steps | Still-open timeline orders **Outcome** before future **Target**; terminal timeline stays chronological. Outcome step is **OutcomeBadge** only; scoring detail via **How we score**. |
