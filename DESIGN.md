...
- **Taxonomy (Category vs Topic):** **Categories** are a **fixed allowlist** (Tech, Sports, Politics, Finance, Weather, Historical) for browse rails and filters. **Topics** are **curated entities** (events, themes) with a slug, display name, and **one or more** categories. Predictions link to topics via `topicIds` (many-to-many); `category` on a prediction is the **primary display** category. Do **not** conflate “topic tab” UI with category — **trending topics** link to `/topics/[slug]`; **category** links on cards and sidebars still use `/category/[slug]`.
- **Forecast discovery cards (Featured + Browse):** Shared **shell** (`rounded-xl`, `bg-surface-elevated`, `p-5`, unified shadow). **Header:** source avatar + name link (left) with **header end** slot — Browse: **outcome badge as filter action** (per-outcome hover tint; **no** per-card pressed ring — filter state lives in **Showing: {status}** + **Clear status filter** with `role="status"`; **44px** min target; keyboard focus ring only on `:focus-visible`); Featured: compact **{accuracy}% accurate** badge (source accuracy + trend color — **not** clickable, **not** market odds). **Title** links to prediction detail — **not** a whole-card link. **Browse only:** muted **Target …** / **Added …** line sits **below the title, above the footer** (not in the footer). **Footer meta row:** muted text-only **category** link to `/category/[slug]` when mapped; Browse adds **· topic** link(s) to `/topics/[slug]` when `topicIds` are set (no category icon chip). Active outcome filter shows **Showing: {status}** with **Clear status filter** under the Browse heading.
- **Featured forecasts (home hero):** Responsive **single-row** card grid inside the elevated hero shell. **Trending topics** strip sits in the hero header: each item is a **real topic** (ranked by linked prediction activity), linking to `/topics/[slug]` — not category strings. **Slot count tracks viewport:** 1 card (&lt; `lg`), 3 (`lg`–`1279px`), 4 (`xl`+); never wrap a second row. Section title: **Featured forecasts** (distinct from the main list **Browse forecasts** below), with one-line **source accuracy / not market prices** disclosure when cards are shown. Pick logic favors **newest per category** then backfill; copy must not imply popularity or live prices.
...
- **Home category tabs:** `CategoryTabs` below the hero filter the **Browse forecasts** list **in place** on `/`. **All** clears the filter (`/` with no `category` query). Other tabs set **`?category={slug}`** (lowercase, e.g. `/?category=finance`) via `history.replaceState` semantics (`router.replace`, `scroll: false`) so filters are **shareable** without leaving home. Dedicated **`/category/[slug]`** pages remain for sidebar feeds, SEO titles, and links from forecast cards. Visual treatment matches existing chip/tab patterns; active state must be obvious for keyboard and screen-reader users. Changing category clears an active outcome status filter and scrolls the browse heading into view (`prefers-reduced-motion` respected).
...
| 2026-05-25 | Home category tabs in place + `?category=` | Tabs filter browse on `/` with shareable `?category={slug}`; `/category/[slug]` kept for deep links and feed layout. |
