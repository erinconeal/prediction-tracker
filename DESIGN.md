...
- **Forecast discovery cards (Featured + Browse):** Shared **shell** (`rounded-xl`, `bg-surface-elevated`, `p-5`, unified shadow). **Header:** source avatar + name link (left) with **header end** slot — Browse: **outcome badge as filter button** (`aria-pressed` when active; **44px** min target); Featured: compact **{accuracy}% accurate** badge (source accuracy + trend color — **not** clickable, **not** market odds). **Title** links to prediction detail — **not** a whole-card link. **Browse only:** muted **Target …** / **Added …** line sits **below the title, above the footer** (not in the footer). **Footer meta row:** muted text-only **category** link to `/category/[slug]` when mapped; Browse adds **· topic** link(s) to `/topics/[slug]` when `topicIds` are set (no category icon chip). Active outcome filter shows **Showing: {status}** with **Clear status filter** under the Browse heading.
