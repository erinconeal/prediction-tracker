...
- **Source detail pages:** `/source/[slug]` uses the same **8+4 grid** at `lg` (timeline main, stats sidebar). **Header:** breadcrumb (`Home / {name}` with `aria-current="page"` on the visible current item), **SourceAvatar** + serif **`h1`**. **Sidebar** (`SourceStatsSidebar`, sticky at `lg`): accuracy card with **SourceAccuracyBadge**, mono percent, **LeaderboardAccuracyBar**, constitution §7.3 context line, total + resolved counts, **How we score** → `/about`. **Timeline:** vertical list of **Browse forecast** cards with **`hideSourceHeader`** + **`readOnlyOutcome`** (title link + read-only outcome badge, timing line, topic footer — no source link, no outcome filter). Empty slug-backed names use **humanized** slug fallback. Outcome scoring stays on prediction detail, not inline on this page.
